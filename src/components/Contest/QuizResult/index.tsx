...
const QuizResult = ({ testResultData }: QuizResultProps) => {
    ...
    const wrongDetails = testResultData?.details?.filter((detail) => !detail.correct);

    const questions =
        correctDetails &&
        wrongDetails &&
        [...correctDetails, ...wrongDetails]?.sort((a, b) => a.questionId - b.questionId);
        ...
        <>
            {testResultData ? (
                <>
                    <Styled.RecordTitle>
                        성적
                        <span> (70% 이상인 경우 녹색으로, 그렇지 않으면 빨간색으로 표시됩니다.)</span>
                    </Styled.RecordTitle>
                    <Styled.ProgressBarContainer>
                        <Styled.ProgressBar score={testResultData?.score} total={testResultData?.total} />
                    </Styled.ProgressBarContainer>
                    <Styled.ScoreIndicator>
                        <Styled.Correct>맞은 개수: {testResultData?.score}</Styled.Correct>
                        <Styled.Wrong>틀린 개수: {testResultData?.total - testResultData?.score}</Styled.Wrong>
                    </Styled.ScoreIndicator>
                                    ...
                                    (detail, index) =>
                                        detail.correct && (
                                            <Styled.ListItem key={index}>
                                                <Styled.QuestionTitle>문제 {index + 1}</Styled.QuestionTitle>
